...
모델 선택(DenseNet121 / MobileNetV3 / EfficientNet-B0) → 사진 선택 또는 카메라 촬영 → **점수 확인** → 실제 모델 예측 점수(1~5)를 확인합니다.
실행 창에서 Ctrl+C를 누르면 종료합니다.

새 PC에서는 Python 3.12 설치 후 `setup_local.cmd`를 한 번 실행합니다.
이 PC에는 `.venv`에 CPU용 실행 환경이 준비되어 있습니다.

모델 폴더 구성 및 교체: [models/README.md](models/README.md)
...
- `GET /api/health`, `GET /api/models`: 세 모델의 준비 상태
- `POST /api/predict?model=densenet121`: 이미지 바이너리 본문 → JSON 점수
